...
class GameRoom:
    ...
    async def broadcast(self, msg_type, data):
        ...
        targets = []
        if self.teacher_ws: targets.append(self.teacher_ws)
...
# 4. Uvicorn 구동 및 포트 개방 확인
import subprocess
import time
import socket
import re

server_proc = subprocess.Popen(["uvicorn", "server:app", "--host", "127.0.0.1", "--port", "8000"])

# 8000번 포트 열릴 때까지 대기 (최대 10초)
for _ in range(20):
...
# 5. Cloudflared 다운로드 및 터널 연결
!wget -nc -q https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64 -O /tmp/cloudflared
...
print("\n" + "=" * 65)
print(f"👉 교사 / 학생 접속 주소: {public_url}")
print("=" * 65)
print("※ 셀이 돌아가는 상태(`[*]`)에서 접속하셔야 정상 작동합니다.\n")
